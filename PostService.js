 ...
 */
  ...
  // --------------------- POST UPDATE -------------------------
  static async UpdatePost(apiContext, postId, postObj = {}) {
    const _m = 'UpdatePost';
    let ecode, updatedPostObj, dbPostObj;
    try {
      //check postObj

      //get post from db
      dbPostObj = await PostHelper.LoadPost(postId);

      //check post from db

      //set post txt

      //set post txt history

      //update post db

      //return 

      let oldText = dbPostObj.getText();

      // Assert props
      postObj = ServiceUtil.FilterScriptForObj(postObj, ['txt']);

      XMPost.AddWord(postObj, 'txt_hist', {
        txt: oldText,
        retired: Date.now()
      });
      ...
      //   updatedPostObj = postObj;
      // }
      updatedPostObj = await PostHelper.UpdatePost(apiContext, dbPostObj, postObj);
    } catch (err) {
      logger.error('error', { err, _m });
      ecode = XError.FromError(err);
    }
